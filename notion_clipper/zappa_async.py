...
from zappa.utilities import get_topic_name
...
class SnsAsyncResponse(LambdaAsyncResponse):
    ...
    def _send(self, message):
        ...
        self.response = self.client.publish(
            TargetArn=self.arn, Message=orjson.dumps(message).decode('utf-8')
        )
...
def route_sns_task(event, context):
    ...
    message = orjson.loads(record["Sns"]["Message"])
    return run_message(message)
    ...
    if message.get("capture_response", False):
        DYNAMODB_CLIENT.put_item(
            ...
            Item={
                ...
                "async_response": {"S": str(orjson.dumps("N/A").decode('utf-8'))},
            },
        )
    ...
    if message.get("capture_response", False):
        DYNAMODB_CLIENT.update_item(
            ...
            ExpressionAttributeValues={
                ":r": {"S": str(orjson.dumps(response).decode('utf-8'))},
                ":s": {"S": "complete"},
            },
        )
...
def get_async_response(response_id):
    ...
    return {
        ...
        "response": orjson.loads(response["Item"]["async_response"]["S"]),
    }
